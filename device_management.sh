...
do
	...
	then
        ...
        ssh -o ConnectTimeout=5 -o StrictHostKeyChecking=no root@$IP -tt "killall kernbypass; if [ $? -gt 0 ]; then echo 'kernbypass not found'; else echo 'kernbypass killed'; fi; sleep 10; sbreload; if [ $? -gt 0 ]; then echo 'failed to reload springboard';else echo 'springboard reloaded'; fi"
